...
from autosprint.app.how_far import (
    _heartbeat_headline,
    run_how_far,
    run_howfar_heartbeat,
)
from autosprint.app.init import _check_install_health, probe_backends, run_doctor
...
def _make_doctor_target(repo: Path) -> None:
    """Add doctor's destination.md check to an already-git target repo."""
    (repo / "autosprint").mkdir()
    (repo / "autosprint" / "destination.md").write_text("x" * 300, encoding="utf-8")


def test_run_doctor_all_green(monkeypatch: pytest.MonkeyPatch, git_target_repo: Path) -> None:
    """A valid setup with a working dispatch round-trip passes — run_doctor does not raise."""
    _make_doctor_target(git_target_repo)
...
def test_run_doctor_exits_nonzero_on_failed_dispatch(monkeypatch: pytest.MonkeyPatch, git_target_repo: Path) -> None:
...
def test_run_doctor_exits_nonzero_when_install_is_stale(monkeypatch: pytest.MonkeyPatch, git_target_repo: Path) -> None:
...
def _make_howfar_target(repo: Path) -> None:
    """Build a target repo with a non-empty destination.md so run_how_far gets past its abort check."""
    (repo / "autosprint").mkdir()
    (repo / "autosprint" / "destination.md").write_text("x" * 300, encoding="utf-8")


def test_run_how_far_dispatches_with_read_only_preset(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    """run_how_far dispatches one agent with the read-only tool preset and the skill instructions."""
    _make_howfar_target(target_repo)
...
def test_run_how_far_aborts_when_destination_missing(target_repo: Path) -> None:
...
def test_run_how_far_unknown_agent_override_raises(target_repo: Path) -> None:
...
def test_run_howfar_heartbeat_appends_log_and_prints_headline(monkeypatch: pytest.MonkeyPatch, target_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    log_path = target_repo / "autosprint" / "logs" / "howfar-heartbeat.log"
...
def test_run_howfar_heartbeat_swallows_dispatch_failure(monkeypatch: pytest.MonkeyPatch, target_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_run_howfar_heartbeat_swallows_missing_destination(target_repo: Path, capsys: pytest.CaptureFixture[str]) -> None:
