"""Tests for the test-runner adapters (pytest + vitest) in autosprint.infra.test_runners.
...
from autosprint.infra.test_runners import (
    PytestRunner,
    VitestRunner,
    _extract_json_objects,
    _run_smoke_with_timeout,
    detect_runner,
    get_test_runner,
)
...
def test_vitest_runner_command_single_package(target_repo: Path) -> None:
...
def test_vitest_runner_command_monorepo(target_repo: Path) -> None:
    """A pnpm-workspace.yaml switches vitest to the `pnpm -r exec` fan-out, serialised."""
    (target_repo / "pnpm-workspace.yaml").write_text("packages:\n  - 'src/*'\n", encoding="utf-8")
...
def test_command_override_applies_to_any_runner(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_detect_runner_pytest_from_pyproject(target_repo: Path) -> None:
...
def test_detect_runner_vitest_from_package_json(target_repo: Path) -> None:
...
def test_detect_runner_python_markers_win_over_package_json(target_repo: Path) -> None:
...
def test_get_test_runner_respects_explicit_vitest(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_vitest_typecheck_command_is_none_without_tsconfig(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_vitest_typecheck_command_is_none_when_disabled(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    """`TS_TYPECHECK=False` disables the gate even when tsconfig.json exists."""
    (target_repo / "tsconfig.json").write_text("{}", encoding="utf-8")
    monkeypatch.setattr(config, "TS_TYPECHECK", False)
    assert VitestRunner()._typecheck_command() is None


def test_vitest_typecheck_command_is_none_when_test_command_overrides(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_vitest_typecheck_defaults_to_npx_tsc(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    (target_repo / "tsconfig.json").write_text("{}", encoding="utf-8")
    (target_repo / "package.json").write_text(json.dumps({"name": "t", "scripts": {"build": "vite build"}}), encoding="utf-8")
    monkeypatch.setattr(config, "TS_TYPECHECK", True)
...
def test_vitest_typecheck_prefers_package_json_script(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    (target_repo / "tsconfig.json").write_text("{}", encoding="utf-8")
    (target_repo / "package.json").write_text(json.dumps({"name": "t", "scripts": {"typecheck": "vue-tsc --noEmit"}}), encoding="utf-8")
    monkeypatch.setattr(config, "TS_TYPECHECK", True)
    cmd = VitestRunner()._typecheck_command()
    assert cmd is not None
...
def test_vitest_typecheck_uses_pnpm_recursive_in_monorepo(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    (target_repo / "tsconfig.json").write_text("{}", encoding="utf-8")
    (target_repo / "pnpm-workspace.yaml").write_text("packages:\n  - 'packages/*'\n", encoding="utf-8")
    (target_repo / "package.json").write_text(json.dumps({"name": "root", "scripts": {"typecheck": "tsc -b"}}), encoding="utf-8")
    monkeypatch.setattr(config, "TS_TYPECHECK", True)
    cmd = VitestRunner()._typecheck_command()
    assert cmd is not None
...
def test_vitest_pre_test_gate_passes_through_when_no_typecheck(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_pytest_smoke_skipped_when_smoke_test_off(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    (target_repo / "pyproject.toml").write_text('[project]\nname = "x"\n', encoding="utf-8")
    (target_repo / "src" / "x").mkdir(parents=True)
    (target_repo / "src" / "x" / "__main__.py").write_text("print('hi')", encoding="utf-8")
    ok, name, _, _ = PytestRunner().post_test_gate()
    assert ok is True
    assert name == ""


def test_pytest_smoke_skipped_when_no_pyproject(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_pytest_smoke_skipped_when_no_main_module(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr(config, "IMPORT_CHECK", False)
    (target_repo / "pyproject.toml").write_text('[project]\nname = "libonly"\n', encoding="utf-8")
    (target_repo / "src" / "libonly").mkdir(parents=True)
    (target_repo / "src" / "libonly" / "__init__.py").write_text("", encoding="utf-8")
    ok, _name, _, _ = PytestRunner().post_test_gate()
    assert ok is True


def test_pytest_detects_package_name_from_pyproject(target_repo: Path) -> None:
    """`_detect_package_name` reads `[project].name` from pyproject.toml."""
    (target_repo / "pyproject.toml").write_text('[project]\nname = "my-game"\nversion = "0.1.0"\n', encoding="utf-8")
    assert PytestRunner()._detect_package_name() == "my-game"


def test_pytest_detect_returns_none_when_no_pyproject(target_repo: Path) -> None:
    """No pyproject → None (not an error). Smoke test skips gracefully."""
    assert PytestRunner()._detect_package_name() is None


def test_pytest_find_main_module_src_layout(target_repo: Path) -> None:
    """src-layout: `src/<pkg>/__main__.py` is detected."""
    (target_repo / "src" / "thepkg").mkdir(parents=True)
    (target_repo / "src" / "thepkg" / "__main__.py").write_text("", encoding="utf-8")
    assert PytestRunner()._find_main_module("thepkg") is True


def test_pytest_find_main_module_flat_layout(target_repo: Path) -> None:
    """Flat layout: `<pkg>/__main__.py` at repo root is also detected."""
    (target_repo / "thepkg").mkdir()
    (target_repo / "thepkg" / "__main__.py").write_text("", encoding="utf-8")
    assert PytestRunner()._find_main_module("thepkg") is True


def test_pytest_find_main_module_returns_false_when_absent(target_repo: Path) -> None:
    """No `__main__.py` in either layout → False (smoke test skips)."""
    (target_repo / "src" / "thepkg").mkdir(parents=True)
    (target_repo / "src" / "thepkg" / "__init__.py").write_text("", encoding="utf-8")
...
def test_run_smoke_with_timeout_passes_on_zero_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.subprocess.run", MagicMock(return_value=mock_proc))
...
def test_pytest_pre_test_gate_all_off_is_noop(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_pytest_format_check_fail_short_circuits(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.subprocess.run", MagicMock(return_value=fail_proc))
...
def test_pytest_format_check_auto_skips_when_black_not_on_path(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr(config, "LINT_CHECK", "off")
    monkeypatch.setattr(config, "PYTEST_COLLECT_GATE", False)
    monkeypatch.setattr("autosprint.infra.test_runners.shutil.which", lambda name: None)
    ok, name, _, _ = PytestRunner().pre_test_gate(quick=False)
    assert ok is True
    assert name == ""


def test_pytest_lint_auto_prefers_ruff_when_pyproject_has_tool_ruff(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.shutil.which", lambda name: f"/usr/bin/{name}")
...
def test_pytest_lint_auto_falls_back_to_flake8_when_no_ruff_config(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    """`LINT_CHECK=auto` picks flake8 when `.flake8` exists and no ruff config is present."""
    (target_repo / ".flake8").write_text("[flake8]\nmax-line-length = 100\n", encoding="utf-8")
    monkeypatch.setattr("autosprint.infra.test_runners.shutil.which", lambda name: f"/usr/bin/{name}" if name == "flake8" else None)
    cmd = PytestRunner()._detect_lint_command()
    assert cmd is not None
    assert "flake8" in cmd[0]


def test_pytest_lint_auto_returns_none_when_nothing_configured(target_repo: Path) -> None:
    """Empty target → `_detect_lint_command` returns None → lint gate skips silently."""
    assert PytestRunner()._detect_lint_command() is None


def test_pytest_collect_only_gate_passes_on_zero_exit(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.subprocess.run", MagicMock(return_value=pass_proc))
    ok, _name, _, _ = PytestRunner().pre_test_gate(quick=False)
    assert ok is True


def test_pytest_collect_only_gate_passes_on_exit_5(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.subprocess.run", MagicMock(return_value=empty_proc))
    ok, _name, _, _ = PytestRunner().pre_test_gate(quick=False)
    assert ok is True


def test_pytest_import_check_passes_when_package_imports(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_pytest_import_check_fails_on_importerror(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
...
def test_pytest_import_check_normalises_hyphen_to_underscore(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
    ...
    monkeypatch.setattr("autosprint.infra.test_runners.subprocess.run", fake_run)
...
def test_pytest_smoke_literal_command_override(monkeypatch: pytest.MonkeyPatch, target_repo: Path) -> None:
