...
def test_record_task_attempt_persists_revert_reason(tmp_repo: Path) -> None:
    db.record_run_start("/x", "builder", "implementor_opus48")
...
def test_schema_is_idempotent_when_db_already_exists(tmp_repo: Path) -> None:
    ...
    db.record_run_start("/y", "duo", "implementor_opus48")
...
def test_schema_migration_adds_recovered_by_fallback_to_legacy_db(tmp_repo: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute("INSERT INTO runs (started_at, target_repo, team, implement_agent) VALUES (?, ?, ?, ?)", ("t0", "/r", "builder", "implementor_opus48"))
    ...
    db.record_run_start("/r2", "builder", "implementor_opus48")
...
def test_connect_does_not_duplicate_runs_db_entry_on_subsequent_connections(tmp_repo: Path) -> None:
    ...
    db.record_run_end("done")
    db._reset_for_tests()
    db.record_run_start("/y", "builder", "implementor_opus48")
    db.record_run_end("done")
    db._reset_for_tests()
    db.record_run_start("/z", "builder", "implementor_opus48")
...
def test_connect_untracks_runs_db_if_already_tracked(tmp_repo: Path) -> None:
    ...
    subprocess.run(["git", "add", "-f", "autosprint/runs.db"], cwd=tmp_repo, check=True, capture_output=True)
    subprocess.run(["git", "commit", "-m", "tracked the blob"], cwd=tmp_repo, check=True, capture_output=True)
    tracked = subprocess.run(["git", "ls-files", "--error-unmatch", "autosprint/runs.db"], cwd=tmp_repo, capture_output=True)
    assert tracked.returncode == 0, "Precondition: runs.db must start as tracked for this test to be meaningful"

    db.record_run_start("/x", "builder", "implementor_opus48")

    still_tracked = subprocess.run(["git", "ls-files", "--error-unmatch", "autosprint/runs.db"], cwd=tmp_repo, capture_output=True)
    assert still_tracked.returncode != 0, "runs.db must no longer be tracked after _connect runs"
    assert db_path.exists(), "Working-tree runs.db must survive — only the git index entry is removed"
...
def test_connect_does_not_error_when_runs_db_is_not_tracked(tmp_repo: Path) -> None:
    ...
    assert (tmp_repo / "autosprint" / "runs.db").exists()
    rows = _query(tmp_repo, "SELECT COUNT(*) FROM runs")
    assert rows == [(1,)]
