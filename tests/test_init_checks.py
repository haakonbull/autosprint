...
def test_verify_target_is_initialised_aborts_when_not_git_repo(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_verify_target_is_initialised_aborts_when_config_toml_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_verify_target_is_initialised_passes_when_both_present(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """When both `.git/` and `autosprint/config.toml` exist, the guard is a no-op."""
    (tmp_path / ".git").mkdir()
    (tmp_path / "autosprint").mkdir()
    (tmp_path / "autosprint" / "config.toml").write_text("# config\n", encoding="utf-8")
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
...
def test_check_python_setup_silent_when_pyproject_present(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_python_setup_warns_when_no_markers_present(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_python_setup_silent_with_just_requirements_txt(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_claude_md_warns_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_claude_md_warns_when_tiny_placeholder(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_claude_md_warns_when_bloated_by_line_count(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_claude_md_warns_when_bloated_by_bytes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
    (tmp_path / "CLAUDE.md").write_text("x" * (60 * 1024), encoding="utf-8")
    init_mod._check_claude_md_and_warn()
    out = capsys.readouterr().out
    assert "⚠" in out
    assert "large" in out
...
def test_check_claude_md_silent_for_normal_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_readme_warns_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_readme_warns_when_tiny_placeholder(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_readme_silent_for_normal_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_bootstrap_target_env_copies_on_yes_and_warns_to_fill_placeholders(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    (tmp_path / ".env.example").write_text("DATABASE_URL=<your-url>\n", encoding="utf-8")
    init_mod._bootstrap_target_env_and_warn()
    assert (tmp_path / ".env").exists()
    assert (tmp_path / ".env").read_text(encoding="utf-8") == "DATABASE_URL=<your-url>\n"
    out = capsys.readouterr().out
    assert "✅" in out
    assert "placeholder" in out


def test_bootstrap_target_env_skips_on_no(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_bootstrap_target_env_silent_when_env_already_exists(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_bootstrap_target_env_silent_when_no_example_present(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_bootstrap_target_env_handles_eof_gracefully(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_dockerignore_silent_for_non_docker_target(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_dockerignore_warns_when_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_dockerignore_warns_on_missing_high_priority_entries(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_check_dockerignore_silent_when_high_priority_covered(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def _git_init_with_files(tmp_path: Path, files: dict[str, str]) -> None:
    ...
    _sp.run(["git", "config", "user.email", "test@example.com"], cwd=tmp_path, check=True)
    _sp.run(["git", "config", "user.name", "test"], cwd=tmp_path, check=True)
    _sp.run(["git", "config", "commit.gpgsign", "false"], cwd=tmp_path, check=True)
    for rel, content in files.items():
        path = tmp_path / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(content, encoding="utf-8")
        _sp.run(["git", "add", rel], cwd=tmp_path, check=True)
    _sp.run(["git", "commit", "-q", "-m", "test"], cwd=tmp_path, check=True)


def test_sensitive_scan_clean_when_repo_has_no_secrets(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
    _git_init_with_files(tmp_path, {"app.py": "def main():\n    return 'hello world'\n", "README.md": "# Demo\n"})
...
def test_sensitive_scan_flags_committed_env_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
    _git_init_with_files(tmp_path, {".env": "DATABASE_URL=postgres://prod\n", "app.py": "print('ok')\n"})
    init_mod._scan_for_sensitive_content_and_warn()
    out = capsys.readouterr().out
    assert "⚠" in out
...
def test_sensitive_scan_flags_aws_access_key_in_tracked_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
    _git_init_with_files(tmp_path, {"config.py": 'AWS_KEY = "AKIAIOSFODNN7EXAMPLE"\n'})
    init_mod._scan_for_sensitive_content_and_warn()
    out = capsys.readouterr().out
    assert "⚠" in out
    assert "AWS" in out
...
def test_sensitive_scan_flags_private_key_block(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr(config, "TARGET_REPO", str(tmp_path))
    _git_init_with_files(tmp_path, {"id_rsa": "-----BEGIN RSA PRIVATE KEY-----\nMIIEow...\n-----END RSA PRIVATE KEY-----\n"})
    init_mod._scan_for_sensitive_content_and_warn()
    out = capsys.readouterr().out
    assert "⚠" in out
